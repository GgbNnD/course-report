...
# 定义余弦退火学习率
lr = nn.cosine_decay_lr(min_lr=0.0001, max_lr=0.01, total_step=epochs*steps_per_epoch, step_per_epoch=steps_per_epoch, decay_epoch=epochs)
...
with open(csv_filename, 'w', newline='') as csvfile:
    fieldnames = ['Epoch', 'Avg Loss', 'Accuracy']
    ...
    for t in range(epochs):
        ...
        test_loss, accuracy = test_loop(network, dataset_eval, net_loss)
        writer.writerow({'Epoch': t+1, 'Avg Loss': test_loss, 'Accuracy': accuracy})
        csvfile.flush()
